...
function enqueue_style(){
    wp_enqueue_script('bootstrap_jquery_js', 'https://cdn.jsdelivr.net/npm/jquery@3.5.1/dist/jquery.slim.min.js');
    ...
    wp_enqueue_style( 'bootstrap_css', 'https://cdn.jsdelivr.net/npm/bootstrap@4.6.1/dist/css/bootstrap.min.css', false, '4.6.1', 'all');
	 wp_enqueue_style( 'fontawesome_6_css', 'https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.2.0/css/all.min.css', false, '4.6.1', 'all');
...
function yoast_remove_canonical_items( $canonical ) {
    if(is_single()){
        ...
        if(get_page_by_path(substr(get_slug(), 0, -2), OBJECT, 'post')){
            if ( $duplicate == "-2" || $duplicate == "-3" || $duplicate == "-4" || $duplicate == "-5") {
                return false;
            }
        }
    }
...
function add_font_awesome_6_cdn_attributes( $html, $handle ) {
    if ( 'fontawesome_6_css' === $handle ) {
        return str_replace( "media='all'", "media='all' integrity='sha512-xh6O/CkQoPOWDdYTDqeRdPCVd1SpvCA9XXcUnZS2FmJNp1coAFzvtCN9BmamE+4aHK8yyUHUSCcJHgXloTyT2A==' crossorigin='anonymous' referrerpolicy='no-referrer' ", $html );
    }
    return $html;
}
